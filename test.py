...
labels = pd.DataFrame({'type': pd.Categorical(pq_labels.read(columns=['type']).columns[0].to_numpy())})
le = LabelEncoder()
...
with tqdm(total=len(labels)) as pbar:
    while True:
        ...
        labels_pred[batch * BATCH_SIZE:batch * BATCH_SIZE + len(matrix)] = le.inverse_transform(y_pred)
        batch += 1
